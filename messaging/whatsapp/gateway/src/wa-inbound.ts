...
export interface EventCounters {
  ...
  media_skipped_total: Record<string, number>;  // sticker (and any future unsupported kinds)
...
export function makeEventCounters(): EventCounters {
  return {
    ...
    media_ok_by_kind: { voice: 0, image: 0, pdf: 0, archive: 0, text: 0, other: 0 },
    media_decrypt_err: 0,
    media_too_long: 0,
    media_too_large: 0,
...
async function handleOne(
  ...
  if (
    ...
    try {
      ...
      if (sock) {
        try {
          const confirmText = "🥷 Ninja: bound. From now on I only listen here.";
          // MUST register BEFORE send — fromMe upsert can arrive before
          // sendMessage resolves, leaking the confirmation into the inbox.
          deps.recentlySent.add(remoteJid, confirmText);
          const sent = await sock.sendMessage(remoteJid, { text: confirmText });
          if (sent?.key?.id) {
            deps.recentlySent.addMessageId(remoteJid, sent.key.id);
          }
        } catch (e) {
  ...
  // Stickers remain unsupported; video/docs now extract as media_kind=other.
  if (!text && !mediaPayload && m.message?.stickerMessage) {
    bumpSkipped(deps, "sticker");
  }
